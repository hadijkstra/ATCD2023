...
class PhIREGANs:
    ...
    DEFAULT_N_EPOCHS = 2 # Number of epochs of training
    ...
    DEFAULT_SAVE_EVERY = 2 # How frequently (in epochs) to save model weights
    DEFAULT_PRINT_EVERY = 50 # How frequently (in iterations) to write out performance
    
    

    def __init__(self, data_type, N_epochs=None, learning_rate=None, epoch_shift=None, save_every=None, print_every=None, mu_sig=None, run_id=None, model_name=None, data_out_path=None):
        ...
        if model_name is None:         
            ...
            self.model_name = model_name + self.run_id
            
        if data_out_path is None:
            self.data_out_path = '/'.join(['data_out', self.run_id])
        else:
            self.data_out_path = data_out_path + self.run_id
        
        
        # Loss array for writing loss values to
        if not os.path.exists('/loss_df'.join([self.model_name, '.csv'])):
            self.loss_df = pd.DataFrame()
        else:
            self.loss_df = pd.read_csv('/loss_df'.join([self.model_name, '.csv']))                
    ...
    def pretrain(self, r, data_path, model_path=None, batch_size=100):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.epoch_shift+1, self.epoch_shift+self.N_epochs+1):
                ...
                vals = {'G loss': '%.3f' % epoch_loss, 'Epoch': str(epoch)}
                self.loss_df = self.loss_df.append(vals, ignore_index = True)
                
                if not os.path.exists(self.model_name):
                    os.makedirs(self.model_name)
                    
                self.loss_df.to_csv('/loss_df'.join([self.model_name, '.csv']), index=False)
    ...
    def train(self, r, data_path, model_path, batch_size=100, alpha_advers=0.001, dropout_rate = 0):
        '''
        ...
        model = SR_NETWORK(x_LR, x_HR, r=r, status='training', alpha_advers=alpha_advers, dropout_rate=dropout_rate)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.epoch_shift+1, self.epoch_shift+self.N_epochs+1):
                ...
                vals = {'G loss': '%.3f' % g_loss, 'D loss': '%.3f' % d_loss, 'Epoch': str(epoch)}
