...
class SR_NETWORK(object):
    def __init__(self, x_LR=None, x_HR=None, r=None, status='pretraining', alpha_advers=0.001, dropout_rate = 0):
        ...
        if status == 'pretraining':
            ...
            self.disc_HR = self.discriminator(self.x_HR, reuse=False, dropout_rate=dropout_rate)
            self.disc_SR = self.discriminator(self.x_SR, reuse=True, dropout_rate=dropout_rate)
    ...
    def discriminator(self, x, reuse=False, dropout_rate = 0):
        N, h, w, C = tf.shape(x)[0], x.get_shape()[1], x.get_shape()[2], x.get_shape()[3]
